...
from utils_func.utils import read_json
from utils_func.data_augmentation import *
from data_loader.dataset import SynchronizedFramesEventsDataset
...
class InferenceDataLoader(DataLoader):
    ...
    def __init__(self, data_path, num_workers=1, pin_memory=True, dataset_kwargs=None,
                 ltype="H5", normalize=False):
        if dataset_kwargs is None:
            dataset_kwargs = {}
        if ltype == "H5":
            dataset = DynamicH5Dataset(data_path=data_path, **dataset_kwargs)
        elif ltype == "H5FR":
            dataset = DynamicH5DatasetHFR(data_path=data_path, **dataset_kwargs)
        elif ltype == "SpadeH5":
            dataset_kwargs['data_path'] = data_path
            dataset = SpadeE2VDataset(dataset_kwargs)
        elif ltype == "MMP":
            dataset = MemMapDataset(data_path, **dataset_kwargs)
        elif ltype == 'NPY':
            dataset = SynchronizedFramesEventsDataset(data_path, event_folder="VoxelGrid-betweenframes-5", flow_folder=None,
                                                      frame_folder='frames', normalize=normalize)
        else:
...
class DynamicH5DatasetHFR(DynamicH5Dataset):
    """
    High framerate version of DynamicH5Dataset
    """
    def __init__(self, **kwargs):
        self.k_framerate = kwargs['voxel_method']['k_framerate']
        self.max_length = kwargs['voxel_method']['max_length']
        assert self.k_framerate >= 1
        super().__init__(**kwargs)

    def compute_frame_indices(self):
        L = len(self.h5_file['images'])
        L = (L - 1) * self.k_framerate + 1
        L = min(L, self.max_length)
        frame_indices = [None]*L
        start_ts = self.t0
        start_idx = 0
        ind = 0
        for i, img_name in enumerate(self.h5_file['images']):
            imgidx = 'images/{}'.format(img_name)
            end_idx = self.h5_file[imgidx].attrs['event_idx']
            ts = self.h5_file[imgidx].attrs['timestamp']
            if self.k_framerate > 1 and i > 0:
                du = ts - start_ts
                end_tses = np.linspace(start_ts, ts, self.k_framerate + 1)[1:-1]
                for end_ts_tmp in end_tses:
                    staet_ts_tmp = end_ts_tmp - du
                    start_idx_tmp = self.find_ts_index(staet_ts_tmp)
                    end_idx_tmp = self.find_ts_index(end_ts_tmp)
                    frame_indices[ind] = [start_idx_tmp, end_idx_tmp]
                    ind += 1
                    if ind >= L:
                        return frame_indices
            frame_indices[ind] = [start_idx, end_idx]
            start_idx = end_idx
            start_ts = ts
            ind += 1
            if ind >= L:
    ...
    def set_voxel_method(self, voxel_method):
        ...
        if self.voxel_method['method'] == 'k_events':
            ...
            self.length = self.num_frames - 1
            self.event_indices = self.compute_frame_indices()
        elif self.voxel_method['method'] == 'k_framerate':
            self.event_indices = self.compute_frame_indices()
...
class SpadeE2VDataset(DynamicH5Dataset):
    def __init__(self, dataset_kwargs):
        self.ev_rate = dataset_kwargs.pop('ev_rate')
    ...
    def __getitem__(self, index, seed=None):
        ...
        if len(xs) < 3:
            voxeles = self.get_empty_voxel_grid(self.combined_voxel_channels).unsqueeze(0)
        else:
            num_vox = int(len(xs) // (self.num_pixels * self.ev_rate))
            if num_vox == 0:
                num_vox = 1
            xses = np.array_split(xs, num_vox, axis=0)
            yses = np.array_split(ys, num_vox, axis=0)
            tses = np.array_split(ts, num_vox, axis=0)
            pses = np.array_split(ps, num_vox, axis=0)

            voxeles = torch.zeros(num_vox, self.num_bins,
                                  self.sensor_resolution[0], self.sensor_resolution[1])
            for i in range(num_vox):
                xs, ys, ts, ps = xses[i], yses[i], tses[i], pses[i]
                try:
                    ts_0, ts_k = ts[0], ts[-1]
                except:
                    ts_0, ts_k = 0, 0
