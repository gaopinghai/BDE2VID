...
from data_loader.dataset import SynchronizedFramesEventsDataset, SynchronizedNPYDataset
...
class InferenceDataLoader(DataLoader):
    ...
    def __init__(self, data_path, num_workers=1, pin_memory=True, dataset_kwargs=None,
        ...
        if ltype == "H5":
            ...
            dataset = SynchronizedFramesEventsDataset(data_path, event_folder="VoxelGrid-betweenframes-5",
                                                      flow_folder=None,
                                                      frame_folder='frames', normalize=normalize)
        elif ltype == 'ENPY':
            dataset = SynchronizedNPYDataset(data_path, event_folder="events", flow_folder=None,
                                             frame_folder='frames', normalize=normalize,
                                             num_bins=dataset_kwargs['num_bins'])
        else:
...
class DynamicH5DatasetHFR(DynamicH5Dataset):
    ...
    def __init__(self, **kwargs):
        self.k_framerate = kwargs.pop('k_framerate', 1)
    ...
    def compute_frame_indices(self):
        ...
        for i, img_name in enumerate(self.h5_file['images']):
            ...
            if self.k_framerate > 1:
                ts = self.h5_file[imgidx].attrs['timestamp']
                start_ts = ts - (ts - start_ts) / self.k_framerate
                start_idx = self.find_ts_index(start_ts)
                start_ts = ts
            frame_indices[i] = [start_idx, end_idx]
            start_idx = end_idx
