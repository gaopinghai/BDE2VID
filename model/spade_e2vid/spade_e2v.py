...
class SPADEE2VID(nn.Module):
    def __init__(self):
        super().__init__()
        self.generator = Unet6().cuda().half()
        self.pred = None
        self.stats = None

    def load_state_dict(self, state_dict, strict=True):
        self.generator.load_state_dict(state_dict, strict)

    def reset_states(self):
        self.pred = None
        self.stats = None

    def BGR2GrayTensor(self, bgr):
        rgb = bgr[:, [2, 1, 0]]
        return rgb_to_grayscale(rgb)

    def forward(self, input_dict):
        x = input_dict['events'].half()
        ...
        for i in range(x.shape[0]):
            xx = x[i][None]
            self.pred, self.stats = self.generator(xx, self.stats, self.pred)
        return {'image': self.BGR2GrayTensor(self.pred)}
