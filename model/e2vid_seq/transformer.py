...
def window_partition(x, window_size):
    ...
    x = x.view(B, D // window_size[0], window_size[0], H // window_size[1], window_size[1], W // window_size[2],
               window_size[2], C)
    # B, D//WS[0], WS[0], H//WS[1], WS[1], W//WS[2], WS[2], C >>>
    # >>> B * D//WS[0] * H//WS[1] * W//WS[2, WS[0] * WS[1] * WS[2], C
...
class WindowAttention3D(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, q_ind=None,
    ...
    def forward(self, x, mask=None):
        ...
        """
        q = x[:, self.q_ind_WinS:self.q_ind_WinE]
        M = q.shape[1]
        B_, N, C = x.shape
        ...
        if mask is not None:
            nW = mask.shape[0]
            mask = mask[:, self.q_ind_WinS:self.q_ind_WinE]  # nW, M, N
            mask = mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(B_ // nW, nW, self.num_heads, M, N)
            attn += mask
            attn = attn.view(-1, self.num_heads, M, N)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(3, 8, 8), shift_size=(0, 0, 0),
        ...
        self.attn = WindowAttention3D(
            dim, window_size=self.window_size, num_heads=num_heads, q_ind=q_ind,
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # merge windows
        window_size_new = (1,) + window_size[1:]
        attn_windows = attn_windows.view(-1, *(window_size_new + (C,)))
        shifted_x = window_reverse(attn_windows, window_size_new, B, 1, Hp, Wp)  # B D' H' W' C
    ...
    def forward(self, x, mask_matrix):
        ...
        shortcut = x[:, self.q_ind:self.q_ind+1]
...
class FrameAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            keys[num_f//2] = x
            x = torch.stack(keys, dim=2)  # B, C, D, H, W
