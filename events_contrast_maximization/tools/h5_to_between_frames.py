...
class BaseVoxelDataset(Dataset):
    ...
    def get_event_voxel(self, index):
        raise NotImplementedError

    def load_data(self, data_path, mode='r'):
        """
    ...
    def make_event_voxel(self, ind):
        dataset_name = "events_voxels_{}/event{:09d}".format(self.num_bins, ind)
        if dataset_name in self.h5_file:
            return
        # if dataset_name in self.h5_file
        idx0, idx1 = self.get_event_indices(ind)
    ...
    def __init__(self, data_path, transforms={}, sensor_resolution=None, num_bins=5,
                 ...
                 filter_hot_events=False, mode='r'):
        """
    ...
    def __getitem__(self, index, seed=None):
        ...
        voxel, dt = self.get_event_voxel(index)
...
class DynamicH5Dataset(BaseVoxelDataset):
    ...
    def get_event_voxel(self, index):
        h5data = self.h5_file[f"events_voxels_{self.num_bins}"]["event{:09d}".format(index)]
        return torch.from_numpy(h5data[:]).float(), h5data.attrs['dt']

    def load_data(self, data_path, mode='r'):
        try:
            self.h5_file = h5py.File(data_path, mode, libver='latest')
...
class SequenceDataset(Dataset):
    ...
    def get_item(self, item_param):
        ind, paused, seed = item_param
        item = self.dataset.__getitem__(ind, seed)
        if paused:
    ...
    def __getitem__(self, i):
        ...
        for u in us:
            ...
            k = k if paused else k + 1
            item_params.append((j+k, paused, seed))
        sequence += [self.get_item(p) for p in item_params]
...
if __name__ == '__main__':
    ...
    args = argparse.ArgumentParser(description='processing datafiles')
    args.add_argument('-d', '--datafiles', default="/media/gph/FILE/Data/event_cnn_min/train.txt", type=str,
                      help='config file path (default: None)')
    args.add_argument('-nb', '--num_bins', default=5, type=int,
                      help='config num_bins (default: 5)')
    args.add_argument('-nw', '--num_workers', default=8, type=int,
                      help='config num_workers (default: 8)')
    args = args.parse_args()

    assert os.path.exists(args.datafiles)
    with open(args.datafiles, 'r') as fp:
        datafiles = fp.read().split('\n')
        datafiles = list(filter(None, datafiles))
    def processor(x):
        DynamicH5Dataset(x, num_bins=args.num_bins, mode='r+')

    list(map(processor, tqdm(datafiles)))
